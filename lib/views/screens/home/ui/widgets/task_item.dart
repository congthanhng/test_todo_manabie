...
import 'package:test_todo_manabie/views/widgets/dialogs/app_dialogs.dart';
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Container(
        child: InkWell(
          onTap: () {
            AppDialog.fDialogShowIOS(context,
                title: item.title,
                onSave: (value) => context.read<TaskBloc>().add(TaskUpdateEvent(
                    TaskModel(
                        taskId: item.taskId,
                        title: value,
                        isDone: item.isDone))));
